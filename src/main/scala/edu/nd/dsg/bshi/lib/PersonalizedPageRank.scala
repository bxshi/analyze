...
object PersonalizedPageRank {
  ...
  def runWithInitialScore[ED: ClassTag](graph:Graph[Double, ED], source: VertexId, maxIter: Int, resetProb: Double = 0.15): Graph[Double, Double] = {
    var rankGraph: Graph[Double, Double] = graph
      // Associate the degree with each vertex
  ...
  def runWithInitialScoreUntilConvergence[ED: ClassTag](
      graph: Graph[Double, ED], source: VertexId, tol: Double, resetProb: Double = 0.15): Graph[Double, Double] =
  {
    val pprGraph: Graph[(Double, Double), Double] = graph
      .outerJoinVertices(graph.outDegrees) {
      (vid, vdata, deg) => (vdata, deg.getOrElse(0))
    }.mapTriplets( e => 1.0 / e.srcAttr._2)
      .mapVertices( (id, attr) => (attr._1, 0.0)).cache()
