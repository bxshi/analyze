...
draw <- function(df, measurevar = "areaUnderROC") {
  # Add ranking
#  cols <- c(1="red",2="blue",3="green",4="orange",5="#E69F00", 6="#56B4E9",
#            7="#009E73", 8="#F0E442", 9="#0072B2", 10="#D55E00")
...
compareAllModels <- function(df) {
  ...
  for(i in 1) {
    ...
    for( j in 1:1) { #dim(models)[2]
      ...
      tmpdf <- df[which(!tmpidx),]
      print(paste("result",length(unique(tmpdf$name))))
      tmpdf.best <- extractRes(tmpdf, "areaUnderROC", 1)
      # Draw this combination
      glist[[length(glist)+1]] <- draw(abbr(tmpdf.best), "areaUnderROC")
    }
  }
  return(glist)
}
